...
- **Radar chart ("Skill Proficiency") labels must never be clipped.** The SVG `viewBox` must be wide enough to contain the edge labels in BOTH languages (the Spanish labels are longer, so size for those). Current value: `viewBox="-11.67 -11.67 343.33 343.33"` (this is the previous fit-everything `-46 -46 412 412` scaled up ×1.2 for a bigger polygon). Each axis label is two lines — name on line 1, percentage on a `.radar-pct` `<tspan>` on line 2 — to keep its horizontal footprint small. **Deliberate exception:** David explicitly chose this ×1.2 size accepting that the two longest Spanish side labels ("Arq / TDD", "Cloud / SDKs") clip slightly at the card edge — do NOT "fix" this by shrinking the chart. The general rule still holds for any NEW labels (keep them short enough to fit). Do NOT widen `.radar-chart-wrapper` / the `.skills-section-layout` columns to enlarge the chart — that grows the frame, which is not wanted. The fit-everything baseline, if ever needed, was `viewBox="-46 -46 412 412"`. If labels change, re-verify the longest ones still fit inside the viewBox.
